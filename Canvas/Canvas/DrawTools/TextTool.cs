...
{
  internal class NodePointText : INodePoint
  {
    public enum ePoint
    {
      Position
    ...
    private Text m_owner;
    private Text m_clone;
    private UnitPoint m_originalPoint;

    private ePoint m_pointId;

    public NodePointText(Text owner, ePoint id)
    {
      m_owner = owner;
      m_clone = m_owner.Clone() as Text;
      m_pointId = id;
    ...
    protected void SetPoint(ePoint pointid, UnitPoint point, Text text)
    {
      if (pointid == ePoint.Position)
        text.Position = point;
    }
  ...
  public class Text : DrawObjectBase, IDrawObject, ISerialize
  {
    ...
    public Text(netDxf.Entities.Text text)
    {
    ...
    public virtual void Copy(Text acopy)
    {
      base.Copy(acopy);
    ...
    public bool PointInObject(ICanvas canvas, UnitPoint point)
    {
      ...
      Pen pp = new Pen(Color.DarkMagenta);
      canvas.Graphics.DrawRectangle(pp, new Rectangle(Point.Round(boundingRectangle.Location), Size.Round(boundingRectangle.Size)));
    ...
    public INodePoint NodePoint(ICanvas canvas, UnitPoint point)
    {
      ...
      if (HitUtil.CircleHitPoint(Position, thWidth, point))
        return new NodePointText(this, NodePointText.ePoint.Position);
      return null;
    }
    ...
    public ISnapPoint SnapPoint(ICanvas canvas, UnitPoint point, List<IDrawObject> otherobjs, Type[] runningsnaptypes, Type usersnaptype)
    {
      ...
      if (runningsnaptypes != null)
      {
        foreach (Type snaptype in runningsnaptypes)
        {
          ...
          {
            Line otherline = UtilsX.FindObjectTypeInList(this, otherobjs, typeof(Line)) as Line;
            if (otherline == null)
              continue;
  ...
  internal class TextEdit : Text, IObjectEditInstance
  {
    ...
    public new void Copy(TextEdit acopy)
    {
      base.Copy(acopy);
