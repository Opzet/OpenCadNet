...
namespace Canvas
{
  public class DataModel : IModel
  {
    ...
    private DrawTools.DrawObjectBase CreateObject(string objecttype)
    {
    ...
    public bool LoadFromNetDxf(netDxf.DxfDocument doc)
    {
      try
      {
        ...
        //TODO 未找到该方法 看字面意思为分解全部块 先注释
        //doc.ExplodeAllBlocks();

        //add lines to drawing
        foreach (Line line in doc.Lines)
        {
          DrawTools.Line l = new DrawTools.Line(new UnitPoint(line.StartPoint.X, line.StartPoint.Y),
                                                new UnitPoint(line.EndPoint.X, line.EndPoint.Y),
                                                0.001f,
                                                Color.White);
    ...
    public List<IDrawObject> GetHitObjects(ICanvas canvas, RectangleF selection, bool anyPoint)
    {
      ...
      foreach (ICanvasLayer layer in m_layers)
      {
        ...
        {
          if (drawobject.ObjectInShape(canvas, UtilsX.GetPointsFromRectangle(selection), anyPoint))
